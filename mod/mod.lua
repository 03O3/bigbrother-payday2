...
function announce(peer,rcode)
	if peer then
		dohttpreq( "http://localhost:8000/webhook/"..peer:user_id().."/"..rcode, function() 
		end )
		log("Webhook sent 1")
	end
end
...
log ("[PDA]  MOD test started for player: "..(tostring(peer:name())))
	for i, mod in ipairs(peer:synced_mods()) do
		...
		local defaultlist = ("pirate perfection,p3dhack,dlc unlocker,skin unlocker,p3dunlocker,selective dlc unlocker,the great skin unlock,mark a cheater,ultimatetrainer")
